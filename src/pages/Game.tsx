...
const Game = () => {
  ...
  const refreshPlayerData = async () => {
    ...
    const { data: craftingJobs, error: craftingJobsError } = await supabase.from('crafting_jobs').select('*').eq('player_id', user.id);

    if (playerDataError || jobsError || craftingJobsError) {
      showError("Erreur lors de la mise à jour des données.");
      console.error(playerDataError || jobsError || craftingJobsError);
    } else {
