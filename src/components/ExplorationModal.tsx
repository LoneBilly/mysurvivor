...
const DetailModal = ({ info, onClose }: { info: PotentialInfo | null, onClose: () => void }) => {
  const { getIconUrl } = useGame();
  if (!info) return null;

  const isEvent = !info.icon?.endsWith('.webp');
  const iconSrc = isEvent ? info.icon : getIconUrl(info.icon);
      ...
      <DialogContent className="bg-slate-800/70 backdrop-blur-lg text-white border border-slate-700">
        <DialogHeader>
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-slate-700/50 rounded-md flex items-center justify-center relative flex-shrink-0 border border-slate-600">
              <ItemIcon iconName={iconSrc} alt={info.name} className="w-10 h-10" />
            </div>
...
const ExplorationModal = ({ isOpen, onClose, zone, onUpdate, onOpenInventory }: ExplorationModalProps) => {
  ...
  const [selectedInfo, setSelectedInfo] = useState<PotentialInfo | null>(null);
  ...
  return (
    <>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <div 
                                  ...
                                  className="relative w-12 h-12 bg-slate-700/50 rounded-md flex items-center justify-center border border-slate-600 cursor-pointer hover:bg-slate-700 transition-colors">
                                  <ItemIcon iconName={getIconUrl(item.icon) || item.icon} alt={item.name} />
                                </div>
                              </TooltipTrigger>
      ...
      <DetailModal info={selectedInfo} onClose={() => setSelectedInfo(null)} />
    </>
  );
};
