...
const GameFooter = ({ stats, credits, onInventaire }: GameFooterProps) => {
      ...
      <footer className="bg-white/5 backdrop-blur-lg border-t border-white/10 text-white p-4">
        ...
        <div className="hidden md:flex items-center">
          ...
          <div className="flex items-center gap-4">
            <Button
              onClick={() => setIsPurchaseModalOpen(true)}
              className="flex items-center justify-center space-x-2 bg-white/10 text-white hover:bg-white/20 rounded-lg border border-white/20 transition-all px-4 py-2"
              variant="default"
            >
              <Coins className="w-5 h-5 text-yellow-400" />
              <span className="font-bold">{credits}</span>
