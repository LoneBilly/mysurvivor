      ...
      return (
        <div className="text-center space-y-4">
          ...
          <div className="w-20 h-20 mx-auto bg-green-500/20 rounded-lg border border-green-500/50 flex items-center justify-center relative"
            data-slot-target="output"
            onMouseDown={(e) => handleDragStart(completedItem, 'output', -1, e.currentTarget as HTMLDivElement, e)}
            onTouchStart={(e) => handleDragStart(completedItem, 'output', -1, e.currentTarget as HTMLDivElement, e)}
          >
            <div className="item-visual w-full h-full">
              {item && <ItemIcon iconName={getIconUrl(item.icon) || item.icon} alt={item.name} />}
            </div>
          </div>
          <p className="text-sm text-gray-400">Glissez l'objet dans votre inventaire pour le récupérer.</p>
        </div>
      );
    }
