...
const WorkbenchModal = ({ isOpen, onClose, construction, onDemolish, onUpdate }: WorkbenchModalProps) => {
  ...
  const handleReturnToInventory = (item: InventoryItem, quantity: number) => {
    setDetailedItem(null);
    const newSlots = [...ingredientSlots];
  
    const slotIndex = newSlots.findIndex(slot => slot?.id === item.id);
    if (slotIndex === -1 || newSlots[slotIndex]!.quantity < quantity) {
      return;
    }
  ...
  return (
    <>
      ...
      <ItemDetailModal
        ...
        onUpdate={onUpdate}
        onTransferToWorkbench={detailedItem?.source === 'inventory' ? handleTransferToWorkbench : undefined}
        onReturnToInventory={detailedItem?.source === 'crafting' ? handleReturnToInventory : undefined}
      />
