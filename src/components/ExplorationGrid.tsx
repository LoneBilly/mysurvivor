...
const ExplorationGrid = ({ playerPosition, onCellClick, onCellHover, path, currentEnergy }: ExplorationGridProps) => {
  ...
  const pathDetails = useMemo(() => {
    if (!path) return { set: new Set<string>(), target: null, cost: 0 };
    return {
        set: new Set(path.map(p => `${p.x},${p.y}`)),
        target: path.length > 1 ? path[path.length - 1] : null,
        ...
        >
          {Array.from({ length: GRID_SIZE }).map((_, y) =>
            Array.from({ length: GRID_SIZE }).map((_, x) => {
              ...
              const isTarget = pathDetails.target?.x === x && pathDetails.target?.y === y;
              const energyCost = pathDetails.cost;
              ...
              const isClickable = (isTarget && energyCost <= currentEnergy) || canClickEntrance;
              ...
              return (
                <ExplorationCell
                  ...
                  isPath={isPath}
                  isAffordablePath={isAffordablePath}
                  isTarget={isTarget}
