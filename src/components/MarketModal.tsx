          ...
          myListings.length > 0 ? (
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {myListings.map(listing => (
                <div key={listing.id} className="flex items-center justify-between p-2 bg-slate-800/60 rounded-md">
                  <div className="flex items-center gap-3">
                    <ItemIcon iconName={listing.items?.icon} alt={listing.items?.name || ''} className="w-8 h-8" />
                    <div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex-1 overflow-y-auto space-y-2 bg-slate-900/50 rounded-lg border border-slate-700 p-2">
          {inventory.length > 0 ? inventory.map(item => (
            <div key={item.id} className="flex items-center justify-between p-2 bg-slate-800/60 rounded-md">
              <div className="flex items-center gap-3">
                <ItemIcon iconName={item.items?.signedIconUrl || item.items?.icon} alt={item.items?.name || ''} className="w-8 h-8" />
                <p className="font-semibold">{item.items?.name} (x{item.quantity})</p>
              </div>
              <Button size="sm" onClick={() => setItemToPrice(item)} disabled={myListings.length >= saleSlots}>Vendre</Button>
            </div>
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl w-full h-[80vh] bg-slate-800/70 backdrop-blur-lg text-white border border-slate-700 shadow-2xl rounded-2xl p-4 sm:p-6 flex flex-col">
          ...
          <Tabs defaultValue="sell" className="flex-1 flex flex-col min-h-0 mt-4">
            ...
            <TabsContent value="buy" className="flex-1 mt-2">
              <div className="h-full flex items-center justify-center bg-slate-900/50 rounded-lg border border-slate-700">
                <p className="text-slate-400">La section d'achat sera bientôt disponible.</p>
              </div>
            </TabsContent>
