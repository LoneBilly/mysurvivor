      ...
      <Card className="bg-gray-700/50 border-gray-600">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-gray-800/50 rounded-lg">
            <div><Label>Zone</Label><select value={selectedZoneId || ''} onChange={(e) => setSelectedZoneId(Number(e.target.value) || null)} className="native-select w-full p-2 bg-gray-800 border border-gray-600 rounded"><option value="">Sélectionner</option>{resourceZones.map(zone => <option key={zone.id} value={zone.id}>{zone.type} ({zone.x}, {zone.y})</option>)}</select></div>
          ...
          <div><Label>Récompense d'objet (si succès)</Label><div className="grid grid-cols-2 gap-4 mt-2"><select value={rewardItem.id?.toString() || ''} onChange={(e) => setRewardItem(prev => ({ ...prev, id: Number(e.target.value) || null }))} className="native-select bg-gray-800 border-gray-600"><option value="">Choisir un objet...</option>{allItems.map(item => <option key={item.id} value={item.id.toString()}>{item.name}</option>)}</select><div><Label className="text-xs">Quantité</Label><Input type="number" min="1" value={rewardItem.quantity} onChange={(e) => setRewardItem(prev => ({ ...prev, quantity: Number(e.target.value) || 1 }))} className="bg-gray-800 border-gray-600" /></div></div></div>
